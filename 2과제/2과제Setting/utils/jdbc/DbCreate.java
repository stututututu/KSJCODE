...
public class DbCreate {
	public DbCreate() {
		...
		Vector<Vector<String>> bookIndex = DbManager.db.getDb("SELECT * FROM 2023지방_2.book;");
		for (Vector<String> row : bookIndex) {
			...
			try {
				...
				DbManager.db.setDb("UPDATE `2023지방_2`.`book` SET `b_img` = ? WHERE (`b_no` = ?);", img, row.get(0));
				System.err.println(row.get(0) + "번 이미지 업데이트 성공");
