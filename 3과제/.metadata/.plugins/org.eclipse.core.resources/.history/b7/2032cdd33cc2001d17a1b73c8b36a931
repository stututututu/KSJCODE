...
public class AirSearchForm extends BaseFrame {
	...
	private Vector<Vector<String>> lastData;
...
//		jcbStart.set
	...
	public void event() {
		...
		jcbStart.addActionListener(e -> {
			...
			lastData = DbManager.db.getDb("SELECT n_name, n_no FROM 2023지방_3.nation where n_no != ?;;", jcbStartIndex);

			for (Vector<String> row : lastData) {
		...
		jlDate.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				int jcbStartIndex = jcbStart.getSelectedIndex();
				int jcbLastIndex = jcbLast.getSelectedIndex();
...
//				}
				...
				AirTicketManager.depart = jcbStartIndex + "";
				AirTicketManager.arrival = lastData.get(jcbLastIndex - 1).get(1);

				new DateChoseForm(airSearchForm, jcbStartIndex + "", lastData.get(jcbLastIndex - 1).get(1));
		...
		jbOkey.addActionListener(e -> {
			...
			AirTicketManager.departString = startLand;
			AirTicketManager.arrivalString = lastLand;
